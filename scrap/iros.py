                    ...
                    if step == 1:  # 1번스텝[테스트열람], 4번스텝[일반열람]에서 인터넷 등기소 alert창이 뜨기때문에 확인함
                        try:
                            ...
                            if step == 1 and mode == 'test':
                                ...
                                self.anySignWaitting()
                                # 열람
                                element = self.driver.find_elements_by_class_name('btn1_n_bg02_action') # 등기소 열람/발급 조회 페이지의 열람 버튼을 선택
                ...
                while mode == 'browse': # 일반 열람 모드인경우
                        ...
                        try:
                            ...
                            while browseWin: # 일반 열람 화면 창이 떠있는 경우
                                ...
                                if browsePrintButton: # 일반 열람 화면 창의 출력 버튼 이미지 서치 성공시
                                    ...
                                    # 인쇄[프린터 선택]창 확인
                                    browsePrintTitle = '인쇄'
                                    browsePrintWin = wait(lambda: getWinStrBegin(browsePrintTitle), timeout_seconds=10) # 인쇄[프린터 선택]창 서치
                    ...
                    try:
                        ...
                        while browsePrintWin:  # 인쇄[프린터 선택]창이 존재하는 경우
                            ...
                            if step == 3:
                                browsePrintDropdownButton = ahk.image_search('search/browse.1.dropdown.png') # 인쇄[프린터 선택]창의 드랍다운 버튼 이미지 서치
                                if browsePrintDropdownButton: # 인쇄[프린터 선택]창의 드랍다운 버튼 이미지 서치 성공시
                                    ...
                                    ahk.mouse_position = (50, 50) # 인쇄[프린터 선택]창의 드랍다운 버튼 클릭 후 50, 50위치로 마우스 커서 이동(마우스 포인터가 드랍다운 메뉴 위에 있는경우 다음에 찾을때 마우스에 가려져서 이미지 서치가 안되는 현상 발생)
                                    sleep(0.5)
                                    step = step + 1  # 다음 step으로 이동
                            ...
                            if step == 4:
                                ...
                                browseSelectPrinterButton1 = ahk.image_search('search/browse.2.printer1.png') # 프린터 모델명 2 이미지
                            ...
                            if step == 7:
                                try:
                                    ...
                                    if browseWin:  # 일반 열람 화면 창을 찾은 경우
                                        ...
                                        if browseWin.activate() is None: # 일반 열람 화면 창이 활성화 되지 않은 경우
                                            browseWin.activate() # 일반 열람 화면 창을 활성화시킴
                                        browseCloseButton = ahk.image_search('search/browse.4.close.png') # 일반 열람 화면 닫기 버튼 이미지 서치
