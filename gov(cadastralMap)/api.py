...
result = ["", "", 0, "cadastralMap"]

app = FlaskAPI(__name__)
CORS(app)

@app.route("/cadastralMap/request", methods=['POST'])
def cadastral_map_download():
    start = datetime.now()
    global result
    ...
    # 정부24
    if callNumber%4 == 0:
          connection = cadastralMap.CadastralMapDownloadPDF(GOV_ID1, GOV_PW1, option)
    elif callNumber%4 == 1:
        connection = cadastralMap.CadastralMapDownloadPDF(GOV_ID2, GOV_PW2, option)
    elif callNumber%4 == 2:
        connection = cadastralMap.CadastralMapDownloadPDF(GOV_ID3, GOV_PW3, option)
    elif callNumber%4 == 3:
        connection = cadastralMap.CadastralMapDownloadPDF(GOV_ID4, GOV_PW4, option)
...
def cadastral_map_status():
    global result
    print(result)
    return result

@app.route("/cadastralMap/reassign", methods=['POST'])
def cadastral_map_reassign():
    start = datetime.now()
    global result
    response = request.data  # data is empty
    print("-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=")
    ...
    try:
        os.rename(f'{download_path}'+'/'+FILE_NAME, f'{download_path}'+'/'+'cadastral_map.pdf')
        finish = datetime.now()
        runningTime = finish - start
        result[2] = 1
        print("######################### Running time: " + str(runningTime))
        ...
        try:
            os.rename(f'{download_path}'+'/'+'cadastral_map.pdf', f'{download_path}'+'/'+'cadastral_map.pdf')
            finish = datetime.now()
            runningTime = finish - start
            result[2] = 1
            print("######################### Running time: " + str(runningTime))
